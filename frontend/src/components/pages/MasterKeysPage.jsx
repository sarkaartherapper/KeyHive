...
export default function MasterKeysPage({ ctx }) {
  ...
  const saveMasterKey = async () => {
    ...
    await api('/api/master-keys', { method: 'POST', body: { provider, api_key: apiKey.trim() } });
    setApiKey(''); setProvider('openai'); setModal(''); notify('Master key saved — encrypted'); loadMasterKeys();
  };

  return <div className='page active'><div style={{ padding: '32px 36px' }}>
    <div className='page-header' style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}><div><div className='page-title'>Master keys</div><div className='page-sub'>Your real provider API keys — stored encrypted, never exposed</div></div><button className='btn btn-primary' onClick={() => setModal('addkey')}>+ Add key</button></div>
    ...
    {!masterKeys.length ? <div className='empty'><div className='empty-icon'>🔑</div><div className='empty-text'>No master keys configured</div><button className='btn btn-primary' onClick={() => setModal('addkey')}>Add your first key</button></div> : masterKeys.map((mk) => <div className='card' key={mk.id} style={{ display: 'flex', alignItems: 'center', gap: '16px' }}><div style={{ width: '40px', height: '40px', background: 'var(--bg4)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>{mk.provider === 'openai' ? '⬛' : '🔵'}</div><div style={{ flex: 1 }}><div style={{ fontSize: '14px', fontWeight: 500, marginBottom: '3px' }}>{mk.provider === 'openai' ? 'OpenAI' : mk.provider}</div><div className='token-val' style={{ fontSize: '12px', color: 'var(--muted)' }}>{mk.key_masked}</div></div><div style={{ fontSize: '12px', color: 'var(--dim)' }}>Added {fmtDate(mk.created_at)}</div><span style={{ fontSize: '11px', background: '#2dca7215', color: 'var(--green)', padding: '3px 9px', borderRadius: '20px' }}>Encrypted</span></div>)}

    <div className={`modal-backdrop ${modal === 'addkey' ? 'open' : ''}`} onClick={(e) => e.target === e.currentTarget && setModal('')}><div className='modal'><div className='modal-title'>Add provider key</div><div className='form-row single'><div className='field'><label>Provider</label><select value={provider} onChange={(e) => setProvider(e.target.value)}><option value='openai'>OpenAI</option></select></div></div><div className='form-row single'><div className='field'><label>API key</label><input type='password' value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder='sk-...' autoComplete='off' /></div></div><div style={{ fontSize: '12px', color: 'var(--muted)', marginTop: '8px' }}>This key will be encrypted immediately. You won't be able to retrieve it — only replace it.</div><div className='modal-footer'><button className='btn btn-ghost' onClick={() => setModal('')}>Cancel</button><button className='btn btn-primary' onClick={saveMasterKey}>Save encrypted key</button></div></div></div>
  </div></div>;
}
